...
REM      %JBOSS_HOME%\bin\ui\
REM Easiest way to make it work is to copy whole "ui" directory to %JBOSS_HOME%\bin
REM
...
REM  v3	2012-09-14 fixed ui log path
...
if exist "%DIRNAME%..\jboss-modules.jar" (
  ...
  REM we are in bin\ui in a WildFly installation
  set "WE=%DIRNAME%..\..\"
  goto :WE_FOUND
) else if exist "%DIRNAME%..\..\..\jboss-modules.jar" (
...
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
  ...
  set PRUNSRV="%JBOSS_HOME%\bin\ui\amd64\wildfly-ui"
) else (
  echo Using the X86-32bit version of prunsrv
  set PRUNSRV="%JBOSS_HOME%\bin\ui\wildfly-ui"
)
...
echo Usage:
echo(
echo   ui install ^<options^>  , where the options are:
echo(
echo     /startup                    : Set the ui to auto start
echo                                 Not specifying sets the ui to manual
echo(
...
echo(
echo     /name ^<servicename^>       : The name of the ui
echo                                 default: %SHORTNAME%
echo(
echo     /display ^<displayname^>    : The display name of the ui, use double
echo                                 quotes to allow spaces.
echo                                 Maximum 256 characters.
echo                                 default: %DISPLAYNAME%
echo(
echo     /desc ^<description^>       : The description of the ui, use double
echo                                 quotes to allow spaces.
...
echo(
echo     /loglevel ^<level^>         : The log level for the ui:  Error, Info,
...
echo(
echo     /debug                      : run the ui install in debug mode
echo(
echo     /properties ^<path^>           : Path of the properties file to use
echo                                 default none
echo(
echo     /environment ^<env^>           : List of environment variables that will be provided to the ui in the form key=value. They are separated using either # or ; characters. If you need to embed either # or ; character within a value put them inside single quotes.
echo                                 default none
echo(
echo Other commands:
echo(
echo   ui uninstall [/name ^<servicename^>]
echo   ui start [/name ^<servicename^>]
echo   ui stop [/name ^<servicename^>]
echo   ui restart [/name ^<servicename^>]
echo(
echo     /name  ^<servicename^>      : Name of the ui: should not contain spaces
echo                                 default: %SHORTNAME%
echo(
goto endBatch
...
if /I "%~1"== "/name" (
  ...
  if "!SHORTNAME!" == "" (
    echo ERROR: You need to specify a ui name
    goto endBatch
  )
  shift
  shift
...
if /I "%~1"== "/display" (
  ...
  if "!DISPLAYNAME!" == "" (
    echo ERROR: You need to specify a ui display name, maximum of 256 characters
    goto endBatch
  )
  shift
  shift
...
if /I "%~1"== "/logpath" (
  ...
  if "!LOGPATH!" == "" (
    echo ERROR: You need to specify a path for the ui log
    goto endBatch
  )
  shift
  shift
...
echo %PRUNSRV% install %SHORTNAME% %RUNAS% --DisplayName="%DISPLAYNAME%" --Description="%DESCRIPTION%" --LogLevel=%LOGLEVEL% --LogPath=%LOGPATH% --LogPrefix=ui --StdOutput=%STDOUT% --StdError=%STDERR% --StartMode=exe --Startup=%STARTUP_MODE% --StartImage=cmd.exe --StartPath=%START_PATH% ++StartParams=%STARTPARAM% --StopMode=exe --StopImage=cmd.exe --StopPath=%STOP_PATH%  ++StopParams=%STOPPARAM% ++Environment=%ENV_VARS%
%PRUNSRV% install %SHORTNAME% %RUNAS% --DisplayName="%DISPLAYNAME%" --Description="%DESCRIPTION%" --LogLevel=%LOGLEVEL% --LogPath=%LOGPATH% --LogPrefix=ui --StdOutput=%STDOUT% --StdError=%STDERR% --StartMode=exe --Startup=%STARTUP_MODE% --StartImage=cmd.exe --StartPath=%START_PATH% ++StartParams=%STARTPARAM% --StopMode=exe --StopImage=cmd.exe --StopPath=%STOP_PATH%  ++StopParams=%STOPPARAM% ++Environment=%ENV_VARS%
...
if errorlevel 8 (
  echo ERROR: The ui %SHORTNAME% already exists
  goto endBatch
)
if errorlevel 0 (
...
if errorlevel 0 (
  ...
  echo Unable to stop the ui %SHORTNAME%
)
goto cmdEnd
...
if errorlevel 2 (
  echo ERROR: Failed to load ui %SHORTNAME% configuration
  goto endBatch
)
if errorlevel 0 (
  goto endBatch
